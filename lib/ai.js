...
import { BID_THRESHOLDS, DIFFICULTY_RANDOM } from './constants';
...
export function getAiBid(hand, currentHighBid, isDealer, allPassedToDealer, difficulty) {
  ...
  let bid = 0;
  if (bestExp >= BID_THRESHOLDS.bid4) bid = 4;
  else if (bestExp >= BID_THRESHOLDS.bid3) bid = 3;
  else if (bestExp >= BID_THRESHOLDS.bid2) bid = 2;

  // Difficulty adjustments
  if (difficulty === 'easy') {
    if (Math.random() < DIFFICULTY_RANDOM.easyUnderbid && bid > 2) bid--;
  } else if (difficulty === 'medium') {
    if (Math.random() < DIFFICULTY_RANDOM.mediumUnderbid && bid > 2) bid--;
  }
...
export function getAiPlay(hand, trumpSuit, trickPlays, seat, /* capturedTricks */ _, difficulty) {
  ...
  // Easy: random play
  if (difficulty === 'easy' && Math.random() < DIFFICULTY_RANDOM.easy) {
