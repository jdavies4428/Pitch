...
export function getValidBids(currentHighBid, isDealer, allPassedToDealer) {
  ...
  const bids = [0]; // 0 = pass
  // Dealer can "steal" the bid by matching; others must exceed
  const minBid = isDealer
    ? Math.max(2, currentHighBid)
