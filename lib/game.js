...
export function isDealerStealBid(bidAmount, currentHighBid, isDealer) {
  return isDealer && bidAmount > 0 && currentHighBid === 4 && bidAmount === currentHighBid;
}

export function isWinningBid(bidAmount, currentHighBid, isDealer) {
  return bidAmount > currentHighBid || isDealerStealBid(bidAmount, currentHighBid, isDealer);
}
...
export function getValidBids(currentHighBid, isDealer, allPassedToDealer) {
  ...
  const bids = [0]; // 0 = pass
  // Dealer can only steal (match) a bid of 4; otherwise must outbid
  const minBid = isDealer && currentHighBid === 4
    ? 4
    : Math.max(2, currentHighBid + 1);
  for (let b = minBid; b <= 4; b++) {
    bids.push(b);
  }
  return bids;
}
...
export function getPlayableCards(hand, trumpSuit, ledSuit) {
  ...
  // Non-trump led: must follow suit if you have it (may also play trump)
...
export function scoreHand(originalHands, capturedTricks, trumpSuit) {
  ...
  // LOW: team that CAPTURED the trick containing the lowest trump
  let low = null;
  if (trumpsDealt.length > 0) {
    const lowest = trumpsDealt.reduce((a, b) => a.rank < b.rank ? a : b);
    for (const trick of capturedTricks) {
      if (trick.cards.some(play => cardEquals(play.card, lowest))) {
        low = getTeam(trick.winner);
        break;
      }
    }
  }
...
export function getLivePoints(originalHands, capturedTricks, trumpSuit) {
  ...
  // HIGH: known from dealt hands (whoever holds it will play it)
  const allDealt = originalHands.flat();
  const trumpsDealt = allDealt.filter(c => c.suit === trumpSuit);
  ...
  if (trumpsDealt.length > 0) {
    ...
    // LOW: determined by who captures the trick containing the lowest trump
    const lowest = trumpsDealt.reduce((a, b) => a.rank < b.rank ? a : b);
    lowCard = lowest;
    for (const trick of capturedTricks) {
      if (trick.cards.some(play => cardEquals(play.card, lowest))) {
