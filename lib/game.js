...
export function getPlayableCards(hand, trumpSuit, ledSuit) {
  ...
  // If trump was led, must follow with trump if possible; otherwise any card
  if (ledSuit === trumpSuit) {
    const trumpCards = hand.filter(c => c.suit === trumpSuit);
    return trumpCards.length > 0 ? trumpCards : [...hand];
  }

  // Non-trump led: must follow suit OR play trump if you have the led suit
  const followCards = hand.filter(c => c.suit === ledSuit);
  if (followCards.length > 0) {
    // Have the led suit: can play led suit or trump
    const trumpCards = hand.filter(c => c.suit === trumpSuit);
    return [...followCards, ...trumpCards];
  }

  // Don't have the led suit: play anything
  return [...hand];
}
